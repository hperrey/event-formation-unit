...
def findStripWithMaxTime(cluster):

    idx = np.argmax(cluster[0])

    return cluster[1][idx]
...
x = np.bincount([], minlength=512)
y = np.bincount([], minlength=512)
...
fig, ax = plt.subplots(2, 2, figsize=(10,5), facecolor='White')
...
while True:
    ...
    x += np.bincount(cluster[0][1], minlength=512)
    y += np.bincount(cluster[1][1], minlength=512)

    xentry.append(findStripWithMaxTime(cluster[0]))
    yentry.append(findStripWithMaxTime(cluster[1]))

ax[0][0].bar(bins, x, 1)
ax[0][1].bar(bins, y, 1)

ax[1][0].hist2d(xentry, yentry, bins=512, range=([[0,512],[0,512]]))
